...
type ProxyRiskSummary struct {
	...
	KnownExitIPCount   int              `json:"known_exit_ip_count"`
	UnknownExitProxies int              `json:"unknown_exit_proxies"`
	TotalBoundAccounts int64            `json:"total_bound_accounts"`
	RiskGroups         []ProxyRiskGroup `json:"risk_groups"`
}

type ProxyRiskGroup struct {
	...
	Proxies       []ProxyRiskProxy `json:"proxies"`
	Warnings      []string         `json:"warnings,omitempty"`
}

type ProxyRiskProxy struct {
	ID             int64  `json:"id"`
...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	if group.RequireOAuthOnly && (IsOpenAICompatiblePlatform(group.Platform) || group.Platform == PlatformAntigravity || group.Platform == PlatformAnthropic || group.Platform == PlatformGemini) && len(accountIDsToCopy) > 0 {
...
func (s *adminServiceImpl) GetProxyRiskSummary(ctx context.Context) (*ProxyRiskSummary, error) {
	...
	summary := &ProxyRiskSummary{
		GeneratedAt: time.Now().Unix(),
		RiskGroups:  make([]ProxyRiskGroup, 0),
	}
	groups := make(map[string]*ProxyRiskGroup)
	for i := range proxies {
		p := proxies[i]
		summary.TotalProxies++
		if p.Status == StatusActive {
			summary.ActiveProxies++
		}
		summary.TotalBoundAccounts += p.AccountCount
		exitIP := proxyExitIP(p)
		if exitIP == "" {
			summary.UnknownExitProxies++
			exitIP = "unknown"
		}
		group := groups[exitIP]
		if group == nil {
			group = &ProxyRiskGroup{
				ExitIP:   exitIP,
				ProxyIDs: make([]int64, 0),
				Proxies:  make([]ProxyRiskProxy, 0),
			}
			groups[exitIP] = group
		}
		group.ProxyCount++
		group.AccountCount += p.AccountCount
		group.ProxyIDs = append(group.ProxyIDs, p.ID)
		group.Proxies = append(group.Proxies, ProxyRiskProxy{
			ID:             p.ID,
			Name:           p.Name,
			Status:         p.Status,
		...
		if group.QualityStatus == "" || proxyQualityRank(p.QualityStatus) > proxyQualityRank(group.QualityStatus) {
			group.QualityStatus = p.QualityStatus
		}
		if group.Country == "" {
			group.Country = p.Country
		}
		if group.CountryCode == "" {
			group.CountryCode = p.CountryCode
		}
	}
	for _, group := range groups {
		if group.ExitIP != "unknown" {
			summary.KnownExitIPCount++
		}
		if group.ExitIP == "unknown" {
			group.Warnings = append(group.Warnings, "exit_ip_unknown")
		}
		if group.AccountCount > 1 {
			group.Warnings = append(group.Warnings, "multiple_accounts_same_exit_ip")
		}
		if group.ProxyCount > 1 {
			group.Warnings = append(group.Warnings, "multiple_proxies_same_exit_ip")
		}
		summary.RiskGroups = append(summary.RiskGroups, *group)
	}
	sort.SliceStable(summary.RiskGroups, func(i, j int) bool {
		left, right := summary.RiskGroups[i], summary.RiskGroups[j]
		if left.AccountCount != right.AccountCount {
			return left.AccountCount > right.AccountCount
		}
		if left.ProxyCount != right.ProxyCount {
			return left.ProxyCount > right.ProxyCount
		}
		return left.ExitIP < right.ExitIP
	})
	return summary, nil
}
...
func (s *adminServiceImpl) saveProxyLatency(ctx context.Context, proxyID int64, info *ProxyLatencyInfo) {
	...
	s.persistProxyExitIP(ctx, proxyID, merged.IPAddress)
}
