...
账号池“后台能测通”不等于消费者拿到的下游 key 能生成。真实验收入口是：

```powershell
$env:SUB2API_CLIENT_KEY="<sub2api-issued downstream key>"
$env:SUB2API_MODEL="claude-sonnet-4-6"
powershell -ExecutionPolicy Bypass -File .\tests\acceptance\verify-downstream-key.ps1
```

该脚本只接受 sub2api 分发给消费者的 key，调用 `https://sub2api.tengokukk.com/v1/chat/completions`，默认同时跑非流式和流式两个请求，并在 `No available accounts`、无正文、无 SSE frame、鉴权失败或调度失败时 fail-closed。不要用上游供应商原始 key 代替这个验收。
