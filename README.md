...
notes live under [`deploy/README.md`](deploy/README.md). Those files are kept
for upstream-compatible self-hosting and local development; they are not this
fork's production release authority.

If a self-hosting path becomes production-authoritative for this fork, update
`project.json`, this section, and the release acceptance command in the same
change.

### Source Development

Build and run from source only for development or customization.
